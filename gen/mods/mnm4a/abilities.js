		...
		onBeforeMove(attacker, defender, move) {
			...
			if (attacker.species.baseSpecies === 'Aegislash' && !attacker.species.isMega) {
				...
				if (attacker.species.falinks !== targetForme) {
					...
					let species = this.getMixedSpecies(baseSpecies, targetForme);
	...
	sos: { // added compatibility for non-Wishiwashi Megas
		...
		onResidual(pokemon) {
			...
			if (!pokemon.species.wishiwashi) {
				let species = this.getMixedSpecies(pokemon.m.originalSpecies, 'Wishiwashi-Mega-1');
				species.wishiwashi = 1;
				pokemon.formeChange(species, this.effect, true);
			} else if (pokemon.species.wishiwashi === 1) {
				let species = this.getMixedSpecies(pokemon.m.originalSpecies, 'Wishiwashi-Mega-2');
				species.wishiwashi = 2;
				pokemon.formeChange(species, this.effect, true);
			} else if (pokemon.species.wishiwashi === 2) {
				let species = this.getMixedSpecies(pokemon.m.originalSpecies, 'Wishiwashi-Mega-3');
				species.wishiwashi = 3;
				pokemon.formeChange(species, this.effect, true);
			} else if (pokemon.species.wishiwashi === 3) {
				let species = this.getMixedSpecies(pokemon.m.originalSpecies, 'Wishiwashi-Mega-4');
				species.wishiwashi = 4;
				pokemon.formeChange(species, this.effect, true);
			} else if (pokemon.species.wishiwashi === 4) {
				let species = this.getMixedSpecies(pokemon.m.originalSpecies, 'Wishiwashi-Mega-School');
