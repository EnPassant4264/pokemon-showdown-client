...
*/
	...
	elegance: {
		...
		onModifyMove(move, attacker) {
			...
			if (_optionalChain([move, 'access', _ => _.self, 'optionalAccess', _2 => _2.chance])) move.self.chance = 100;
		},
		name: "Elegance",
		rating: 5,
		num: -43,
	...
	cheapheat: {
		...
		onBeforeMove(source, target, move) {
			...
			if (move.useTargetOffensive) {
				...
				let cheapHeatBoostSource = {};
				let cheapHeatBoostTarget = {};
	...
	gyaru: {
		...
		onModifyType(move, pokemon) {
			...
			if (move.type === 'Fighting' && !noModifyType.includes(move.id) && !(move.isZ && move.category !== 'Status')) {
				move.type = 'Fire';
				(move ).gyaruBoosted = true;
			}
		},
		onBasePowerPriority: 23,
		onBasePower(basePower, pokemon, target, move) {
			if ((move ).gyaruBoosted) return this.chainModify([0x1333, 0x1000]);
		},
		name: "Gyaru",
		rating: 4,
		num: -69,
