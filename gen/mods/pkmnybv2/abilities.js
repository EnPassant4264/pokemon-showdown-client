   ...
   burningechoes: {
	...
	owntempo: {
		...
		onHit(target, source, move) {
			if (_optionalChain([move, 'optionalAccess', _ => _.volatileStatus]) === 'confusion') {
	...
	galewings: {
      shortDesc: "This Pokemon's Flying-type moves have +1 priority when this Pokemon is above 50% HP.",
		onModifyPriority(priority, pokemon, target, move) {
			if (_optionalChain([move, 'optionalAccess', _2 => _2.type]) === 'Flying' && pokemon.hp >= pokemon.maxhp / 2) return priority + 1;
		},
		name: "Gale Wings",
		rating: 3,
		num: 177,
